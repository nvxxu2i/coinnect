...
impl BittrexApi {
	...
	fn public_query(
		...
		let url = "https://bittrex.com/api/v1.1".to_string()
			+ method + "?"
	...
	fn private_query(
		...
		let base_url = "https://bittrex.com/api/v1.1".to_string()
			+ method + "?apikey="
			+ &self.api_key
			+ "&nonce=" + &nonce;
		...
		let mut res = match self.http_client.post(&url).headers(custom_header).send() {
			Ok(res) => res,
			Err(err) => return Err(ErrorKind::ServiceUnavailable(err.to_string()).into()),
		};
