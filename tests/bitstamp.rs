...
mod bitstamp_tests {
	...
	fn ticker_should_have_the_correct_last() {
		...
		assert_ne!(
			result.unwrap().last_trade_price,
	...
	fn order_book_should_have_asks_for_btcusd() {
		...
		let mut api = BitstampApi::new(creds).unwrap();
		assert!(api
			.return_order_book(Pair::BTC_USD)
			.unwrap()
			.contains_key("asks"));
	}
	#[test]
	...
	fn should_create_a_correct_signature() {
		...
		let expected_signature =
			"7D7C4168D49CBC2620A45EF00EAA228C1287561F1C1F94172272E1231A8ADF6B".to_string();
		assert_eq!(
			utils::build_signature(nonce, customer_id, api_key, api_secret).unwrap(),
			expected_signature
		);
	}
